...
public class MezclaNatural {
    ...
    // 1. meta()
    void goal() { 
        String title = "MEZCLA NATURAL";
        String message = """
            Programa que permite ordenar datos ingresados mediante teclado
            o archivo mediante el metodo de ordenamiento "Mezcla natural".
    ...
    // 3.1. ordenar() -> Algoritmo interno de mezcla natural
    private void naturalMerge(int[] array, int n) {
        int[] aux = new int[n];
        ...
        while (!ordenado) {
            ...
            while (i < n) {
                ...
                if (i + 1 < n) comparaciones++; 
                int end1 = i;

                if (i + 1 >= n) {
                    for (int x = start1; x <= end1; x++) {
                        aux[k++] = array[x]; movimientos++;
                    } break;
                }
                ordenado = false;
                int start2 = i + 1; i++;
                ...
                while (p <= end1 && q <= end2) {
                    comparaciones++; 
                    if (array[p] <= array[q]) {aux[k++] = array[p++];} 
                    else {aux[k++] = array[q++];}
                    movimientos++;
                }
                while (p <= end1) {aux[k++] = array[p++]; movimientos++;}
                while (q <= end2) {aux[k++] = array[q++]; movimientos++;} i++;
            }
            for (i = 0; i < n; i++) {array[i] = aux[i]; movimientos++;}
        }
    }
    ...
    void process() { 
        ...
        ep.outInfo("Procesando ordenamiento por mezcla natural...", "MEZCLA NATURAL");
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            switch (option) {
                ...
                case 1 -> mn.printMenu();
                case 2 -> mn.emptyData(); 
                case 3 -> { mn.ep.outInfo("Ejecución finalizada.", "SALIR");
                            return;}
                default -> {}
            }
        }
