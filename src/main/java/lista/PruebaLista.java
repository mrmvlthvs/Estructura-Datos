...
----- CLASE PRINCIPAL DE PRUEBA METODOLÓGICA (MY ARRAY LIST) -----
*/
package lista;
...
public class PruebaLista {
    ...
    private MyList<String> masterList = new MyArrayList<>();

    // 1. meta()
    void goal() {
        String title = "PRUEBA METODOLOGICA: MY ARRAY LIST";
        String msg = """
            Programa para evaluar operaciones del ADT MyArrayList (Liang).
            Permite ingresar nombres completos mediante teclado o archivo
            acumulandolos en memoria, y probar operaciones de insercion, 
            remocion, busqueda y reemplazo de la estructura.
            """;
    ...
    void proceso() {
        if (masterList.isEmpty()) {
            in.outAdv("La lista esta vacia. Registre datos.", "AVISO");
            return;
        }
        String opt[] = {"Add (Indice)", "Remove (Indice)", "Remove (Valor)",
                        "Buscar (Contains)", "Set (Reemplazar)"};
        ...
        try {
            switch(sel) {
                ...
                case 1 -> {
                    ...
                    if (idx != null) masterList.remove((int)idx);
                }
                case 2 -> {
                ...
                case 3 -> {
                    ...
                    if (val != null) {
                        ...
                        int pos = masterList.indexOf(val.trim());
                        String res = "¿Contiene? " + ok + "\nIndice: " + pos;
                        in.outInfo(res, "BUSQUEDA");
                    }
                }
                case 4 -> {
                    Integer idx = capturarEntero("SET", "Indice a modificar:");
                    if (idx == null) return;
                    String val = in.entryString("SET", "Nuevo nombre:");
                    if (val != null) masterList.set(idx, val.trim());
                }
            }
        } catch (Exception e) {
    ...
    void vaciarLista() {
        ...
        if (in.entryOption(msg, "VACIAR", new String[]{"Si", "No"}) == 0) {
            masterList.clear(); // Uso del metodo nativo del ADT
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            int sel = app.in.entryOption("Accion:", "MENU PRINCIPAL", opts);
